...
class MediaSessionManagerImplBase implements MediaSessionManagerImpl {
    ...
    public boolean isTrustedForMediaControl(@NonNull RemoteUserInfoImpl userInfo) {
        ...
        try {
            ...
            if (!isPermissionGranted(userInfo, PERMISSION_STATUS_BAR_SERVICE)) {
                if (!isPermissionGranted(userInfo, PERMISSION_MEDIA_CONTENT_CONTROL)) {
                    if (userInfo.getUid() != 1000) {
                        if (!isEnabledNotificationListener(userInfo)) {
                            return z;
                        }
                    }
                }
    ...
    boolean isEnabledNotificationListener(@NonNull RemoteUserInfoImpl userInfo) {
        ...
        if (enabledNotifListeners != null) {
            ...
            for (ComponentName component : components) {
                ...
                if (component2 != null) {
                    if (component2.getPackageName().equals(userInfo.getPackageName())) {
                        return true;
                    }
                }
            }
