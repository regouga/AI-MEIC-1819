...
class FindAddress {
    ...
    private static class ZipRange {
        ...
        boolean matches(String zipCode) {
            ...
            if ((this.mLow > prefix || prefix > this.mHigh) && prefix != this.mException1) {
                if (prefix != this.mException2) {
                    return false;
                }
            }
            return true;
    ...
    private static boolean isValidZipCode(String zipCode, MatchResult stateMatch) {
        ...
        if (sZipCodeRe.matcher(zipCode).matches()) {
            if (sStateZipCodeRanges[stateIndex].matches(zipCode)) {
                z = true;
                return z;
            }
        }
            ...
            if (matchHouseNumber(content, it) == null) {
                ...
                if (!isValidLocationName(matcher.group(0))) {
                    if (wordCount == 5 && !foundLocationName) {
                        it = matcher.end();
                        break;
                    } else if (foundLocationName && wordCount > 4) {
